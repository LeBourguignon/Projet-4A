...
export class DevLevelInTheDarkness extends Level {
    constructor(app: Application<ICanvas>) {
        ...
        var background = new StaticColorBlock({coordinate: {x: -16*32, y: 0}, width: 3*16*32, height: 9*32}, 0x262626);
        ...
        super(app, {size: {coordinate: {x: -16*32, y: 0}, width: 3*16*32, height: 9*32}, drawables: [background, floor, block1, block2, block3, tenemigs, player], player: player, obstacles: [floor, block1, block2, block3], camCoordinate: new Coordinate({x: 0, y: 0}), inTheDarkness: true});
    }
