...
import { Player, playerHeight } from "../hitBox/player";
...
export class DevLevelInTheDarkness extends Level {
    constructor(id: number = 0) {
        ...
        const player = new Player({x: -16*32, y: 8*32-playerHeight});
        ...
        super({size: {coordinate: {x: -32*32, y: 0}, width: 4*16*32, height: 9*32}, drawables: [background, floor, block1, block2, block3, tenemigs, player, torch], player: player, obstacles: [floor, block1, block2, block3], camCoordinate: new Coordinate({x: 0, y: 0}), inTheDarkness: true}, id);
    }
