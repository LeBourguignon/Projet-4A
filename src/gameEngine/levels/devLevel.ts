...
export class DevLevel extends Level {
    constructor(id: number = 0) {
        ...
        var floor = new StaticColorBlock({coordinate: {x: -16*32, y: 8*32}, width: 4*16*32, height: 32}, 0x262626);
        ...
        super({size: {coordinate: {x: -16*32, y: 0}, width: 4*16*32, height: 9*32}, drawables: [background, floor, block1, block2, block3, tenemigs, player], player: player, obstacles: [floor, block1, block2, block3], camCoordinate: new Coordinate({x: 0, y: 0}), inTheDarkness: false}, id);
    }
