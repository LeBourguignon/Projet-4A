...
export const firePlace01Width = 41;
export const firePlace01Height = 41;

export const firePlace01SpriteTime = 10;

export const firePlace01LightingRadius = 3*64;

export class FirePlace01 extends HitBox {
    #showHitBox: boolean;
    ...
    #spriteTime: number = firePlace01SpriteTime;
    ...
    constructor(coordinate: Coord, showHitBox: boolean = false) {
        super({coordinate: coordinate, width: firePlace01Width, height: firePlace01Height});
        ...
        this.#textures = [
                        assets.light.fireplace01Off,    // 0
                        assets.light.fireplace01On00,   // 1
                        assets.light.fireplace01On01,   // 2
                        assets.light.fireplace01On02,   // 3
                        assets.light.fireplace01On03,   // 4
                        assets.light.fireplace01On04,   // 5
                        assets.light.fireplace01On05    // 6
                    ];
    ...
    addLighting(level: Level, lighting: Graphics) {
        if(this.#state) {
            lighting.beginFill(0xFF0000)
                    .drawCircle(this._coordinate.x + this._width/2 - level.camCoordinate.x + level.size.coordinate.x, this._coordinate.y + this._height/2 - level.camCoordinate.y + level.size.coordinate.y, firePlace01LightingRadius)
                    .endFill();
        }
    }
    ...
    #updateAnimation(level: Level, delta: number) {
        ...
        if(!this.#state) {
            ...
            if(this.#spriteTime < 0) {
                this.#spriteTime = firePlace01SpriteTime;
